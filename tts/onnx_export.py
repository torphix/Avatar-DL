...
from PortaSpeech.utils.model import get_model
from PortaSpeech.utils.tools import get_configs_of
import torch
import argparse

# took that script from: https://github.com/ccoreilly/wav2vec2-service/blob/master/convert_torch_to_onnx.py

def convert_to_onnx(model_path, dataset_name, onnx_model_name):
    print(f"Converting {model_path} to onnx")
    
    configs = get_configs_of(dataset_name)
    model = get_model(model_path, configs, 'cpu')
